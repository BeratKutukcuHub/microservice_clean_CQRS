...
namespace Category.Infrastructure.Repositories
{
    public class AuditRepository : IAuditRepository
    {
        ...
        public async Task<bool> AddAuditLogAsync(AuditLog auditLog)
        {
            ...
            {
                throw new System.Exception("Failed to add audit log", ex);
            }
        }
        public async Task<bool> AddAuditLogsAsync(List<AuditLog> auditLogs)
        {
            ...
            {
                throw new System.Exception("Failed to add audit logs", ex);
            }
        }
        public async Task<List<AuditLog>> GetAuditLogAsync(List<System.Guid> Id)
        {
            try
            {
            ...
            {
                throw new System.Exception("Failed to get audit logs", ex);
            }
        }
        public async Task<PaginationResponse<AuditLog>> GetAuditLogsAsync(PaginationValue pag)
        {
            try
            {
                ...
                var items = await _collection.Find(FilterDefinition<AuditLog>.Empty)
                    ...
                    .Limit(pag.PageSize)
                    .ToListAsync();
                var totalPages = (int)System.Math.Ceiling(totalCount / (double)pag.PageSize);
